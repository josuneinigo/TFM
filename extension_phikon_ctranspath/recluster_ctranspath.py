"""
recluster_ctranspath.py

A partir de los embeddings de CTransPath (ctranspath_features.npy +
ctranspath_patch_names.txt), aplica k-means (k=8) y genera los ficheros de
...
Uso:
    python recluster_ctranspath.py
"""
...
FEATURES_DIR = os.path.join(BASE, 'data', 'features_ctranspath')
OUT_DIR = os.path.join(BASE, 'data', 'labels', 'K8_ctranspath')
K = 8
...
def main():
    ...
    feats = np.load(os.path.join(FEATURES_DIR, 'ctranspath_features.npy'))
    with open(os.path.join(FEATURES_DIR, 'ctranspath_patch_names.txt')) as f:
