"""
evaluate_patient_level_ctranspath.py

Evalua los checkpoints de select_clusters.py entrenados sobre los clusters
de CTransPath (K8_ctranspath), agregando las predicciones por paciente
(voto mayoritario) sobre la union de val+test de cada cluster.

A diferencia de evaluate_patient_level_valtest.py (usado para el baseline),
aqui no hace falta filtrar test/*.txt por ruta: recluster_ctranspath.py ya
genero directamente los ficheros <cluster>_test.txt separados por cluster.

Uso:
    python evaluate_patient_level_ctranspath.py 00 01 02 03 04 05 06 07
"""
...
LABELS_DIR = os.path.join(BASE, 'data', 'labels', 'K8_ctranspath')
...
def evaluate_cluster(cluster, device):
    ...
    ckpt_path = os.path.join(CKPT_DIR, f'K8_ctranspath_{cluster}_resnet50.pth')
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python evaluate_patient_level_ctranspath.py <cluster> [<cluster> ...]")
        sys.exit(1)
